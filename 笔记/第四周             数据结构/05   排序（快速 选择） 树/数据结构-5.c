...
深度（高度）：节点的层数等于父节点的层数加一，根节点的层数定义为一。树中节点层数的最大值称为该树的高度或深度
...
满二叉树：  具有k层的二叉树，如果节点的个数为 2k-1 个（2的k次方减1个）。那么这棵树称之为满二叉树。
完全二叉树：满足两个条件的二叉树。(1) 只有最下面两层有度数不为2的节点。  (2)最下面一层的节点，都集中在左边的若干位置上。 


*2. 二叉树的存储  

1) 顺序存储：   有n个节点的完全二叉树可以用有 n+1 个元素的数组进行顺序存储，
